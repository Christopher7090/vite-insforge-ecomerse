...
### SDK vs REST API Access Pattern

- **Use SDK** (`insforge.database`, `insforge.auth`) for: products, categories, orders, cart, current user auth
- **Use REST API** (`/api/auth/users` with API key) for: reading admin user data, user listing, profile updates on other users
- Admin user endpoints (`GET /api/auth/users`, `PATCH /api/auth/profiles/{userId}`) require `Authorization: Bearer {API_KEY}` header
- REST API returns `{ users: [...], total }` for listing, `{ user: {...} }` for single user
- Profile updates on other users use `PATCH /api/auth/profiles/{userId}` with `{ profile: { rol, nombre, ... } }`
- **CRITICAL**: The SDK queries the `public` schema. `auth.users` is in the `auth` schema, so `insforge.database.from("auth.users")` will always fail — use REST API instead.
...
- **Project:** **testproject** (API base `https://y74su2bv.us-east.insforge.app`)
